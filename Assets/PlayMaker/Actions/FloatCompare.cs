...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class FloatCompare : FsmStateAction
	{
		...
		void DoCompare ()
		{
			// equal != null is add by zomzhi
			if (Mathf.Abs (float1.Value - float2.Value) <= tolerance.Value && equal != null)
			{
				Fsm.Event (equal);
				return;
			}

			// lessThan != null is add by zomzhi
			if (float1.Value < float2.Value && lessThan != null)
			{
				Fsm.Event (lessThan);
				return;
			}
