...
public class MasterShaderLocalSpaceEditor : MaterialEditor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (isVisible && !shdr.hasMultipleDifferentValues && shdr.objectReferenceValue != null)
        {
            ...
            RangeProperty(GetMaterialProperty(targets, "_ShadowPower"), "Realtime shadow power");
            EditorGUILayout.Separator();

            EditorGUILayout.HelpBox("Local space shader doesn't support Dynamic Batching and mesh combining. If you are reusing " +
                                    "materials, consider using regular (world-space) MasterShader instead.", MessageType.Info);
