...
namespace MyCompany.MyGame.Audio
{
	public class BeatDetection : MonoBehaviour
	{
		...
		/// <summary>
		/// Low pass frequency... frequencies less than 60 are tricky
		/// </summary>
		private int minFrequency = 60;
		/// <summary>
		/// Number of frequency divisions. 50 is ok
		/// </summary>
		private const int MAX_FRQSEC = 50;
		/// <summary>
		/// Linear divisions per octave
		/// </summary>
		private const int LINEAR_OCTAVE_DIVISIONS = 3;
		/// <summary>
		/// Real number of history buffer per frequency.
		/// </summary>
		private const int HISTORY_LENGTH = 43;
		/// <summary>
		/// Allocation buffer. Must be greater than MAX_FRQSEC*LINEAR_OCTAVE_DIVISIONS*HISTORY_LENGTH
		/// </summary>
		private const int MAX_HISTORY = 500;
		/// <summary>
		/// Minimum beat separation in time
		/// </summary>
		...
		float[] spectrum0_history;
		float[] frames0_history;

		// Root mean square of raw data (volume, but not in dB).
		float rms, rms_history;

		public void SetAudioSource (AudioSource curAudioSource)
		{
		...
		void Start ()
		{
			...
			spectrum0_history = new float[numSamples];
			frames0 = new float[numSamples];
			frames1 = new float[numSamples];
			frames0_history = new float[numSamples];

			setUpEnergy ();
			setUpFrequency ();
			//beat检测计时器，防止两次beat发生间隔太短
			tIni = Time.time;
